...
struct Crops{
	...
	unsigned short x_coord[100000];
	unsigned short y_coord[100000];
	//unsigned short COM_x;
	//unsigned short COM_y;
};


struct Crops MetaDataArray[5000];
...
main(argc, argv)
	...
	char *argv[];
	{
		int y,x,i,j,yin,xin,Area_T;              /* index counters               */
		...
		for (y = im.ylo ; y <= im.yhi ; y++) {
			for (x = im.xlo; x <= im.xhi; x++)  {
				if(tm.u[y][x] == 255 && im.u[y][x]==0){ //object label found and current output label not set	
					if(redo_label){
						set_label(x,y,tm.u[y][x],redo_label);
						redo_label=false;
					}else{
						set_label(x, y, tm.u[y][x], false);
					}
					if(MetaDataArray[L-1].area<Area_T){
						redo_label = true;
					}
