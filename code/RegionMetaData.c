...
struct Crops MetaDataArray[1000];
	...
	char *argv[];
	{
		...
		for (y = im.ylo ; y <= im.yhi ; y++) {
			for (x = im.xlo; x <= im.xhi; x++)  {
				im.s[y][x]=0; //clear output image
				tmf.s[y][x]=0;//clear other output
			}
		}
		for (i = 0 ; i < 1000 ; i++) {
		...
		for (y = im.ylo ; y <= im.yhi ; y++) {
			for (x = im.xlo; x <= im.xhi; x++)  {
				if(tm.s[y][x] < 0 && im.s[y][x]==0){ //object label found and current output label not set
...
void set_label(int x, int y, int L, bool redo)
{
	...
	if(tm.s[y+1][x]<0 && im.s[y+1][x]==0){
		set_label(x, y+1, L,false);
	}
	if(tm.s[y-1][x]<0 && im.s[y-1][x]==0){
		set_label(x, y-1, L,false);
	}
	if(tm.s[y][x+1]<0 && im.s[y][x+1]==0){
